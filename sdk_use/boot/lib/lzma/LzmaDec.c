      ...
      }
      {
        ...
        else
        {
          ...
          else
          {
            ...
            confine = (1 << kLenNumHighBits);
          }
        }
        TREE_DECODE(probLen, confine, len);
        len += offset;
      }
    ...
    ELzmaFinishMode finishMode, ELzmaStatus *status)
{
  ...
  *status = LZMA_STATUS_NOT_SPECIFIED;
  ...
  while (p->remainLen != kMatchSpecLenStart)
  {
      ...
      {
        SizeT processed;
        const Byte *bufLimit = HI_NULL;
        if (inSize < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow)
        {
          int dummyRes = LzmaDec_TryDummy(p, src, inSize);
          if (dummyRes == DUMMY_ERROR)
          {
            memcpy_s(p->tempBuf, inSize, src, inSize);
  ...
  *srcLen = *destLen = 0;
  for (;;)
  {
    ...
    *srcLen += inSizeCur;
    ...
    memcpy_s(dest, outSizeCur, p->dic + dicPos, outSizeCur);
    dest += outSizeCur;
    outSize -= outSizeCur;
    *destLen += outSizeCur;
    if (res != 0)
