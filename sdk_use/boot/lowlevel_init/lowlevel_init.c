...
#define W_REG_BIT_MASK		((unsigned int)0x1f << W_REG_BIT_OFFSET)
...
static inline void reg_read(struct regentry *reg, unsigned int *ret)
{
	...
	bit_start_r = (reg->attr & ((unsigned int)R_REG_BIT_MASK))>>((unsigned int)R_REG_BIT_OFFSET);
...
//c00132816  modify for read without check.
   if(reg->attr&R_WHETHER_PM)
...
static inline void reg_write(struct regentry *reg)
{
	...
	if(32 == bit_num_w){
		...
		reg_val_w &= (unsigned int)(~((((unsigned int)1<<bit_num_w)-1)<<bit_start_w));
...
void init_registers(struct regentry reg_table[],unsigned int pm)
{
	unsigned int i;
