...
#define MRS_STA_RM_CFG_LOCAL    (0)     // Local, STA/CltII <--> Meter, CltI Module <--> CltI
#define MRS_STA_RM_CFG_CLTI     (1)     // CltI transfer, CltI Module <--> CltI <--> Meter
#define MRS_STA_RM_CFG_AUTO     (0xFF)  // Auto mode, choose 'Local' or 'CltI transfer' mode by product type
...
typedef struct
{
    ...
    HI_U32 ulMeterPowerOffTime;     // 电表(上1次)掉电开始时刻
...
/* BEGIN: PN: DTS2017_698.45 MODIFY\ADD\DEL by cuiate/00233580 at 2017/6/28 */
    ...
    HI_U8   ucFrmProto;         // 当前处理的报文类型

    HI_U8   ucDutAdcFlag : 1;
    HI_U8   pad1 : 7;
    HI_U8   aucCode[2];
/* END:   PN: DTS2017_698.45 MODIFY\ADD\DEL by cuiate/00233580 at 2017/6/28 */
    ...
    HI_U8   ucSoftTfMode;       // 无扰台区识别模式: 0关闭，1打开(白名单常关，站点自由切换网络)，2打开(白名单常开，站点不主动切换网络)
    ...
    MRS_STA_RM_CFG_STRU stRmCfg;    // STA rm config

    MRS_STA_POWER_INFO_STRU stPowerInfo;    // 停电信息
...
HI_BOOL mrsStaIsMrFrameMatch645(HI_U8 * pDlFrame, HI_U16 usDlLength, HI_U8 * pUlFrame, HI_U16 usUlLength);
HI_BOOL mrsStaIsMrFrameMatch698(HI_U8 * pDlFrame, HI_U16 usDlLength, HI_U8 * pUlFrame, HI_U16 usUlLength);
...
HI_U32 mrsGetDeviceIdProcFromUart(MRS_645_FRAME_STRU *pst645Frame);
#endif
HI_END_HEADER
