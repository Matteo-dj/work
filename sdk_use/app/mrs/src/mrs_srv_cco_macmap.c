...
HI_U32 mrsMapMaintain(MRS_MAP_MODULE *module)
{
    ...
    if (HI_FALSE == bFirst)
    {
        ...
        if (!maclist->number)
        {
            ucTurnFlag = HI_TRUE;//当环境为全载环境且都以表地址入网时,cco状态直接由映射表维护状态变为正常状态
        }
    }
...
HI_U32 mrsMapAddMacMeter188(MRS_MAP_MODULE *module,HI_U8 mac[],HI_U8 uca188Mac[],HI_U8 ucType)
{
    MRS_MAC_DEVICE_PAIR *pItem = HI_NULL; 
    HI_U16 index = 0;
    HI_S16 sindex = -1;  //空闲的位置
    HI_S16 smindex = -1; //空闲电表位置
    HI_S16 nonactindex = -1; //老化电表位置
