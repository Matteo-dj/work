...
{
    HI_U32 ret = HI_ERR_SUCCESS;
    PROTO376_2_FRAME_AFN_AND_DATA_STRU afnData;
    ...
    ret = mrsCreate3762UpFrame(&afnData, &buf, &bufLen, HI_NULL, HI_NULL);
    if (HI_ERR_SUCCESS == ret)
    {
    ...
    HI_DIAG_LOG_MSG_E1(MRS_FILE_LOG_FLAG_001, HI_DIAG_MT("cco send req time frame"), ret);
    mrsToolsFree(buf);
...
//    mrsSetFristGetTimeFlg(MRS_FIRST_ACQUIRE_TIME_END);//optimized by weichao
}
...
HI_U32 mrsLogTimerProc(HI_VOID)
{
    ...
    // CCO 0点或12点时,将抄表日志存入flash
    if ((0 == tmHour) || (12 == tmHour))
    {
...
HI_VOID mrsLogXrToCache(MRS_SRV_MRSLOG_CONTEXT *ctx)
{
    ...
    (hi_void)memcpy_s(&(HI_SDM_MRS_LOG_GPTR->xrStats), sizeof(HI_SDM_ENG_MRS_XR_STATS_S), &(ctx->xrStats), sizeof(ctx->xrStats));
}
...
/* BEGIN: PN: DTS2015042907341 MODIFY\ADD\DEL by cuiate/00233580 at 2015/4/13 */
    ...
    mrsMeterAddrPreProc(aucAddr, addr, METER_PROTO_645_1997);
...
HI_BOOL mrsGetCcoTimeFrist(HI_VOID)
{
    ...
    mrsSetFristGetTimeFlg(MRS_FIRST_ACQUIRE_TIME_DOING);//optimized by weichao
