...
#define MRS_RM_METER_RETRY_CNT_MAX              (2)             // 电表端超时重传次数(最大)
#define MRS_RM_TIMEOUT_RETRY_CNT_DEFAULT        (0)             // 电表端超时重传次数(默认)
#define MRS_RM_DENY_RETRY_CNT_DEFAULT           (0)             // 电表端否认重传次数(默认)
#define MRS_RM_METER_RETRY_CNT_MIN              (0)             // 电表端超时重传次数(最小)

#define MRS_PRM_METER_RETRY_CNT_DEFAULT         (1)             // 并发抄表时电表端超时重传次数
#define MRS_RM_METER_RETRY_CNT_DEFAULT          (0)             // 点抄轮抄抄表时电表端超时重传次数
...
#define MRS_RM_PARALLEL_FAIL_CNT_MAX            (10)             // 并发读表失败次数超过10时算异常电表
#define MRS_RM_PARALLEL_FAIL_SEND_ONE           (10)             // 异常电表并发读表时，读10次，实际读表一次，其他直接返回读表失败
...
//异常电表节点
typedef struct MRS_RM_FAIL_NODE_UE
{
    MRS_SRV_LINK link;
    HI_U8 ucMeterAddr[HI_PLC_MAC_ADDR_LEN];   // 目的站点MAC地址
    HI_U8 ucFailNum;    // 异常电表读取失败次数，暂定为连续失败10次为异常电表
    HI_U8 ucAllNum;    // 集中器请求数据的次数，暂规定为异常电表请求数据10次中发送一次
} MRS_RM_FAIL_NODE, *P_MRS_RM_FAIL_NODE;

typedef struct
{
    HI_U8 ucMeterAddr[HI_PLC_MAC_ADDR_LEN];   // 目的站点MAC地址
    HI_U16 pad;
    MRS_SRV_LINK* pstLink;
} MRS_RM_FAIL_FIND_LINK;
...
// PLC包接收
HI_PUBLIC HI_U32 mrsRmPlcPackageReceive(MRS_DATA_TRANSMIT_AFNF1_UP_STRU *, HI_U16, MRS_DEVICE_INF_STRU *, HI_U8 ucMacAdd[HI_PLC_MAC_ADDR_LEN]);
...
HI_PUBLIC HI_VOID mrsRmRetryCfgInit(P_MRS_RETRY_CFG_S, NV_MRS_RETRY_CFG_S *, HI_U8);
/* END:   PN: DTS2015033100426 MODIFY\ADD\DEL by cuiate/00233580 at 2015/3/26 */

HI_PUBLIC HI_VOID mrsRmAddFailNode(HI_U8 *ucMeterAddr);
HI_PUBLIC HI_BOOL mrsRmFindMeterListNode(HI_VOID * pstMeterListNode, HI_VOID * pstExist);
HI_PUBLIC HI_VOID mrsParallelRmFailDel(HI_U8* MeterAddr);
HI_PUBLIC HI_U32 mrsParallelFailMeterBack(P_MRS_RM_REQ_DATA pstData);
HI_PUBLIC HI_U32 mrsParallelPlcRespProc(P_MRS_RM_REQ_DATA pstData);
