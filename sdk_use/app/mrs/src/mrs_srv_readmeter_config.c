...
HI_U32 mrsRmCfgParamInit(NV_MRS_RETRY_CFG_S *pstCfg)
{
    ...
    NV_MRS_RETRY_CFG_S *pLRInf = HI_NULL;
		...
		PLC_OVERTIME_INF_STRU *pstAfn13Saved = HI_NULL;
#endif
...
#if defined(MRS_SRV_PROTOCOL_CJT188)
		pstAfn13Saved = mrsGetPlcOverTimeInfSavedAfn13();
		mrsToolsMemcpy_s(pstAfn13Saved, sizeof(PLC_OVERTIME_INF_STRU),pInf, sizeof(PLC_OVERTIME_INF_STRU));
#endif
...
HI_U32 mrsDiagGetMrRetryCfg(HI_U16 usId, HI_PVOID pCmdParam, HI_U16 usCmdParamSize, HI_U32 ulOption)
{
...

    {
        ...
        if (HI_ERR_SUCCESS == ulRet)
        {
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_XR]), &(stPlcInf.stPlcOverTimeParam[MRS_RM_XR]), MRS_RM_XR);
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_LR]), &(stPlcInf.stPlcOverTimeParam[MRS_RM_LR]), MRS_RM_LR);
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_PR]), &(stPlcInf.stPlcOverTimeParam[MRS_RM_PR]), MRS_RM_PR);
        }
        else
        {
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_XR]), HI_NULL, MRS_RM_XR);
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_LR]), HI_NULL, MRS_RM_LR);
            mrsRmRetryCfgInit(&(pInd->astRetryCfg[MRS_RM_PR]), HI_NULL, MRS_RM_PR);
        }
