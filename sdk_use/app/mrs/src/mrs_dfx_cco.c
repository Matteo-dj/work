...
HI_U32 mrsCcoDfxSendSearchInfoExpToHso(HI_U16 usId, HI_U32 ulOption)
{
    DIAG_CMD_CCO_SEARCH_INFO_EXP_IND* pstInfo = HI_NULL;
    MRS_CCO_SRV_CTX_STRU* pstCcoCtx = mrsCcoGetContext();

    pstInfo = mrsToolsMalloc(sizeof(DIAG_CMD_CCO_SEARCH_INFO_EXP_IND));
    if (HI_NULL == pstInfo)
    {
        return HI_ERR_NOT_ENOUGH_MEMORY;
    }

    (hi_void)memset_s(pstInfo, sizeof(DIAG_CMD_CCO_SEARCH_INFO_EXP_IND), 0, sizeof(DIAG_CMD_CCO_SEARCH_INFO_EXP_IND));

    pstInfo->ucInterfaceVer = MRS_DFX_CCO_SEARCH_INFO_IND_VER0;
    pstInfo->ucStruSize = (HI_U8)sizeof(DIAG_CMD_CCO_SEARCH_INFO_EXP_IND);
    pstInfo->ucSearchStatus = (HI_U8)((pstCcoCtx->status & MRS_CCO_STATUS_SEARCH_METER) >> 4);
    if ((pstCcoCtx->status & MRS_CCO_STATUS_SEARCH_METER_MASK) == MRS_CCO_STATUS_SEARCH_METER_TRANSFORMER)
    {
        pstInfo->ucOption = MRS_CCO_STATUS_SEARCH_MODE_TRANSFORMER;
    }
    pstInfo->usMacListNum       = pstCcoCtx->stAutoSearchModel.stMacList.num;
    pstInfo->usUnlockStaNum     = (HI_U16)mrsSrvListLength(&pstCcoCtx->stAutoSearchModel.stUnLockStaList);
    pstInfo->usUnreportStaNum   = (HI_U16)mrsSrvQueueLength(&pstCcoCtx->stAutoSearchModel.stMeterQueue);
    pstInfo->usReportedStaNum   = pstCcoCtx->stAutoSearchModel.stSearchDfx.usTotal;
    pstInfo->bQueryComplete     = pstCcoCtx->stAutoSearchModel.bQueryComplete;
    pstInfo->bLockComplete      = pstCcoCtx->stAutoSearchModel.bLockComplete;

    HI_MDM_ReportPacket(usId, (HI_U8)ulOption, (HI_PBYTE)pstInfo, sizeof(DIAG_CMD_CCO_SEARCH_INFO_EXP_IND), HI_TRUE);
...
HI_VOID mrsCcoDfxUpdateSearchResultList(METER_NODE* pstNode, HI_U16 usSeq)
{
    ...
    else if (MRS_DEVICE_TYPE_METER == pstNode->type)
    {
        ...
        (hi_void)memcpy_s(pstMeter->aucAsset, HI_METER_ADDR_LEN, pstNode->asset, HI_METER_ADDR_LEN);
...
// 获取停电检测维测信息
HI_U32 mrsCcoDfxSendPoweroffInfoToHso(HI_U16 usId, HI_VOID* pCmdParam, HI_U32 ulOption)
{
#if defined (PRODUCT_OPTIMIZE_MATTEO)
    DIAG_CMD_MRS_CCO_POWEROFF_INFO_IND_STRU stInd = {0};
    MRS_NODE_STATE_CHG_REPORT_CTX * pstCtx = mrsGetChgReportCtx();

    HI_UNREF_PARAM(usId);
    HI_UNREF_PARAM(pCmdParam);

    (hi_void)memset_s(&stInd, sizeof(DIAG_CMD_MRS_CCO_POWEROFF_INFO_IND_STRU), 0,sizeof(DIAG_CMD_MRS_CCO_POWEROFF_INFO_IND_STRU));
    ...
    else
    {
        ...
        stInd.ulGetPoweroffInfoSeq      = pstCtx->ulGetPoweroffInfoSeq;
        stInd.usMacReportOffSum         = pstCtx->usMacReportOffSum;
        stInd.usMacReportOnSum          = pstCtx->usMacReportOnSum;
        stInd.usDetectSuccessSum        = pstCtx->usDetectSuccessSum;
        stInd.usInterceptSum            = pstCtx->usInterceptSum;
        stInd.us3762ReportOffSum        = pstCtx->us3762ReportOffSum;
        stInd.us3762ReportOnSum         = pstCtx->us3762ReportOnSum;
        stInd.usRecheckFailSum          = pstCtx->usRecheckFailSum;
        stInd.usMacReportCorrectSum     = pstCtx->usMacReportCorrectSum;
        stInd.usMacReportWrongSum       = pstCtx->usMacReportWrongSum;
        stInd.usMacReportMissSum        = pstCtx->usMacReportMissSum;
        stInd.usDetectReportCorrectSum  = pstCtx->usDetectReportCorrectSum;
        stInd.usDetectReportWrongSum    = pstCtx->usDetectReportWrongSum;
        stInd.usDetectReportMissSum     = pstCtx->usDetectReportMissSum;
        stInd.us3762ReportCorrectSum    = pstCtx->us3762ReportCorrectSum;
        stInd.us3762ReportWrongSum      = pstCtx->us3762ReportWrongSum;
        stInd.us3762ReportMissSum       = pstCtx->us3762ReportMissSum;
        stInd.bDetectComplete           = pstCtx->pstCheckStateList->bDetectComplete;
        stInd.bRecheckComplete          = pstCtx->pstCheckStateList->bRecheckComplete;
        stInd.usDetectingNum            = mrsPoweroffGetDetectingNum(pstCtx);
        stInd.usRecheckingNum           = mrsPoweroffGetRecheckingNum(pstCtx);
        stInd.usDetectIdx               = pstCtx->pstCheckStateList->usDetectIdx;
        stInd.usRecheckIdx              = pstCtx->pstCheckStateList->usRecheckIdx;
    }

    HI_MDM_ReportPacket(ID_DIAG_CMD_MRS_CCO_GET_POWEROFF_INFO, (HI_U8)ulOption, (HI_PBYTE)&stInd,
                        sizeof(DIAG_CMD_MRS_CCO_POWEROFF_INFO_IND_STRU), HI_FALSE);
#endif
    return HI_ERR_SUCCESS;
}

// 获取站点的停电维测信息
HI_U32 mrsCcoDfxSendStaPoweroffInfoToHso(HI_U16 usId, HI_VOID* pCmdParam, HI_U32 ulOption)
{
#if defined (PRODUCT_OPTIMIZE_MATTEO)

    DIAG_CMD_MRS_STA_POWEROFF_INFO_REQ_STRU * pstReq = (DIAG_CMD_MRS_STA_POWEROFF_INFO_REQ_STRU *)pCmdParam;
    DIAG_CMD_MRS_STA_POWEROFF_INFO_IND_STRU stInd = {0};
    ...
    (hi_void)memset_s(&stInd, sizeof(DIAG_CMD_MRS_STA_POWEROFF_INFO_IND_STRU), 0,sizeof(DIAG_CMD_MRS_STA_POWEROFF_INFO_IND_STRU));

    pstItem = mrsGetPoweroffCheckItem(mrsGetChgReportCtx(), pstReq->mac);
    if (pstItem)
    {
        stInd.ulRet = HI_ERR_SUCCESS;

        stInd.ucStatus                  = pstItem->ucStatus;
        stInd.ucReCheckStatus           = pstItem->ucReCheckStatus;
        stInd.ucDetectQueryCnt          = pstItem->ucDetectQueryCnt;
        stInd.ucRecheckQueryCnt         = pstItem->ucRecheckQueryCnt;
        stInd.ucReportCheckScore        = pstItem->ucReportCheckScore;
        stInd.ucRecheckFailCnt          = pstItem->ucRecheckFailCnt;
        stInd.usMacReportOffCnt         = pstItem->usMacReportOffCnt;
        stInd.usMacReportOnCnt          = pstItem->usMacReportOnCnt;
        stInd.bPassedDetect             = pstItem->bPassedDetect;
        stInd.ucMacReportWrongCnt       = pstItem->ucMacReportWrongCnt;
        stInd.ucMacReportMissCnt        = pstItem->ucMacReportMissCnt;
        stInd.ucMacReportCorrectCnt     = pstItem->ucMacReportCorrectCnt;
        stInd.ucDetectReportWrongCnt    = pstItem->ucDetectReportWrongCnt;
        stInd.ucDetectReportMissCnt     = pstItem->ucDetectReportMissCnt;
        stInd.ucDetectReportCorrectCnt  = pstItem->ucDetectReportCorrectCnt;
        stInd.uc3762ReportWrongCnt      = pstItem->uc3762ReportWrongCnt;
        stInd.uc3762ReportMissCnt       = pstItem->uc3762ReportMissCnt;
        stInd.uc3762ReportCorrectCnt    = pstItem->uc3762ReportCorrectCnt;
        stInd.ulLastMacReportOffTime    = pstItem->ulLastMacReportOffTime;
        stInd.ulLastMacReportOnTime     = pstItem->ulLastMacReportOnTime;
        stInd.ulLastReportOffTime       = pstItem->ulLastReportOffTime;
        stInd.ulLastReportOnTime        = pstItem->ulLastReportOnTime;
        stInd.ulMeterLastPoweroffTime   = pstItem->ulMeterLastPoweroffTime;
        stInd.ulMeterLastPoweronTime    = pstItem->ulMeterLastPoweronTime;
        stInd.ulLastMacReportWrongTime  = pstItem->ulLastMacReportWrongTime;

        HI_MDM_MakeSecondToCalendar(pstItem->ulLastMacReportOffTime,    &stInd.stLastMacReportOffTime);
        HI_MDM_MakeSecondToCalendar(pstItem->ulLastMacReportOnTime,     &stInd.stLastMacReportOnTime);
        HI_MDM_MakeSecondToCalendar(pstItem->ulLastReportOffTime,       &stInd.stLastReportOffTime);
        HI_MDM_MakeSecondToCalendar(pstItem->ulLastReportOnTime,        &stInd.stLastReportOnTime);
        HI_MDM_MakeSecondToCalendar(pstItem->ulMeterLastPoweroffTime,   &stInd.stMeterLastPoweroffTime);
        HI_MDM_MakeSecondToCalendar(pstItem->ulMeterLastPoweronTime,    &stInd.stMeterLastPoweronTime);
    }
    else
    {
        stInd.ulRet = HI_ERR_FAILURE;
    ...
    HI_MDM_ReportPacket(ID_DIAG_CMD_MRS_CCO_GET_STA_POWEROFF_INFO, (HI_U8)ulOption, (HI_PBYTE)&stInd,
                        sizeof(DIAG_CMD_MRS_STA_POWEROFF_INFO_IND_STRU), HI_FALSE);
#endif
    return HI_ERR_SUCCESS;
}
...
// 获取停电上报最近的历史记录
HI_U32 mrsCcoDfxSendPoweroffHistoryToHso(HI_U16 usId, HI_VOID* pCmdParam, HI_U32 ulOption)
{
#if defined (PRODUCT_OPTIMIZE_MATTEO)

    DIAG_CMD_MRS_POWEROFF_HISTORY_REQ_STRU * pstReq = (DIAG_CMD_MRS_POWEROFF_HISTORY_REQ_STRU *)pCmdParam;
    DIAG_CMD_MRS_POWEROFF_HISTORY_IND_STRU stInd = {0};
    MRS_NODE_STATE_CHG_REPORT_CTX * pstCtx = mrsGetChgReportCtx();
    HI_U32 ulIdx = 0, ulIndIdx = 0;
    ...
    (hi_void)memset_s(&stInd, sizeof(DIAG_CMD_MRS_POWEROFF_HISTORY_IND_STRU), 0,sizeof(DIAG_CMD_MRS_POWEROFF_HISTORY_IND_STRU));

    if (!pstCtx || !pstCtx->pstHistoryList)
    {
        stInd.ulRet = HI_ERR_FAILURE;
    }
    else
    {
        stInd.ulRet = HI_ERR_SUCCESS;
        stInd.ucIndex = pstCtx->pstHistoryList->ucIndex;

        for (ulIndIdx = 0; ulIndIdx < DIAG_CMD_MRS_GET_POWEROFF_HISTORY_NUM; ulIndIdx++)
        {
            ulIdx = (pstReq->begin_index + ulIndIdx) % MRS_POWEROFF_CHECK_HISTORY_NUM;

            (hi_void)memcpy_s(stInd.astRecords[ulIndIdx].aucMac,HI_PLC_MAC_ADDR_LEN, pstCtx->pstHistoryList->astRecords[ulIdx].aucMac, HI_PLC_MAC_ADDR_LEN);
            stInd.astRecords[ulIndIdx].ucStatus                 = pstCtx->pstHistoryList->astRecords[ulIdx].ucStatus;
            stInd.astRecords[ulIndIdx].ulMacReportOffTime       = pstCtx->pstHistoryList->astRecords[ulIdx].ulMacReportOffTime;
            stInd.astRecords[ulIndIdx].ulMacReportOnTime        = pstCtx->pstHistoryList->astRecords[ulIdx].ulMacReportOnTime;
            stInd.astRecords[ulIndIdx].ulMeterLastPoweroffTime  = pstCtx->pstHistoryList->astRecords[ulIdx].ulMeterLastPoweroffTime;
            stInd.astRecords[ulIndIdx].ulMeterLastPoweronTime   = pstCtx->pstHistoryList->astRecords[ulIdx].ulMeterLastPoweronTime;
        }
    }

    HI_MDM_ReportPacket(ID_DIAG_CMD_MRS_CCO_GET_POWEROFF_HISTORY, (HI_U8)ulOption, (HI_PBYTE)&stInd,
                        sizeof(DIAG_CMD_MRS_POWEROFF_HISTORY_IND_STRU), HI_FALSE);
#endif
    return HI_ERR_SUCCESS;
}
...
// 获取停电上报流程中的信息
HI_U32 mrsCcoDfxSendPoweroffingInfoToHso(HI_U16 usId, HI_VOID* pCmdParam, HI_U32 ulOption)
{
#if defined (PRODUCT_OPTIMIZE_MATTEO)

    DIAG_CMD_MRS_POWEROFFING_INFO_REQ_STRU * pstReq = (DIAG_CMD_MRS_POWEROFFING_INFO_REQ_STRU *)pCmdParam;
    DIAG_CMD_MRS_POWEROFFING_INFO_IND_STRU stInd = {0};
    MRS_NODE_STATE_CHG_REPORT_CTX * pstCtx = mrsGetChgReportCtx();
    MRS_POWEROFF_CHECK_ITEM_S * pstItems = HI_NULL;
    HI_U32 ulIdx = 0, ulRecordIdx = 0, ulIndIdx = 0;
    ...
    (hi_void)memset_s(&stInd, sizeof(DIAG_CMD_MRS_POWEROFFING_INFO_IND_STRU), 0,sizeof(DIAG_CMD_MRS_POWEROFFING_INFO_IND_STRU));
    ...
    else
    {
        ...
        {
            if (0 == pstItems[ulIdx].ucStatus && 0 == pstItems[ulIdx].ucReCheckStatus)
            {
                continue;
            }

            if ((0xFF == pstReq->ucDetectStatus || pstItems[ulIdx].ucStatus == pstReq->ucDetectStatus)
                && (0xFF == pstReq->ucRecheckStatus || pstItems[ulIdx].ucReCheckStatus == pstReq->ucRecheckStatus))
            {
                if (ulRecordIdx >= pstReq->usBeginIndex)
                {
                    (hi_void)memcpy_s(stInd.astMacList[ulIndIdx].aucMac,HI_PLC_MAC_ADDR_LEN, pstItems[ulIdx].aucMac, HI_PLC_MAC_ADDR_LEN);
                    ulIndIdx++;
                }

                ulRecordIdx++;

                if (ulIndIdx >= DIAG_POWEROFFING_QUERY_NUM)
                {
                    break;
                }
            }
    ...
    HI_MDM_ReportPacket(ID_DIAG_CMD_MRS_CCO_GET_POWEROFFING_INFO, (HI_U8)ulOption, (HI_PBYTE)&stInd,
                        sizeof(DIAG_CMD_MRS_POWEROFFING_INFO_IND_STRU), HI_FALSE);
#endif
    return HI_ERR_SUCCESS;
}
