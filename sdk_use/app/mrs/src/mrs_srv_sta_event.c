...
HI_PRV HI_U32 mrsStaCreateEvtReportPlc698(HI_VOID);
...
HI_PRV HI_U32 mrsStaEvtNvInit(HI_U16 usId)
{
    ...
    if (first)
    {
        ret = HI_MDM_GetRealRandom(0,HI_ALL_F_32,&ulRandom);
        if (HI_ERR_SUCCESS == ret)
        {
            pstCtx->usSeq = (HI_U16)ulRandom;
        }
        first = HI_FALSE;
    }
...
HI_U32 mrsStaEvtMeterChgNotify(HI_BOOL bChg, HI_U8 ucSlaveFlag, HI_U32 ulParam)
{
    ...
    if(ucSlaveFlag == STA_EQUIP_CHECK_MODE){
		mrs_power_failure_set_equipmode_flag(ucSlaveFlag);
	}
...
HI_U32 mrsStaSendQueryFrame(HI_U16 usId, HI_U8 *pFrame, HI_U16 usFrameLength)
{
    ...
    pItem->ucProtocol = METER_PROTO_645_2007;
...
HI_PRV HI_U32 mrsStaCreateEvtReportPlc(HI_IN HI_PBYTE pInBuf, HI_IN HI_U16 usInLen)
{
    ...
    if (!pstPlcFrame)
    {
        //HI_DIAG_LOG_MSG_E0(MRS_FILE_LOG_FLAG_002, HI_DIAG_MT("Malloc failed"));//optimization by weichao
    ...
    if (!pucPayload)
    {
        //HI_DIAG_LOG_MSG_E0(MRS_FILE_LOG_FLAG_003, HI_DIAG_MT("Malloc failed"));//optimization by weichao
...
HI_U32 mrsStaCreateEvtReportPlc698(HI_VOID)
{
    MRS_PLC_FRAME_DATA_S *pstPlcFrame = HI_NULL;
    MRS_EVENT_INFO_STRU *pstEvtInfo = HI_NULL;
    MRS_STA_SRV_CTX_STRU *pstStaCtx = mrsStaGetContext();
    ...
    if (!pstStaCtx || !pstEvtCtx)
    {
        return HI_ERR_NO_INITILIZATION;
    }

    if (!mrsToolsNormalAddr(pstStaCtx->aucMeter))
    {
    ...
    pstEvtInfo = (MRS_EVENT_INFO_STRU *)mrsToolsMalloc(sizeof(MRS_EVENT_INFO_STRU));
    if (!pstEvtInfo)
    {
        mrsToolsFree(pstPlcFrame);
        return HI_ERR_MALLOC_FAILUE;
    ...
    (hi_void)memset_s(pstPlcFrame, sizeof(MRS_PLC_FRAME_DATA_S), 0, sizeof(MRS_PLC_FRAME_DATA_S));
    (hi_void)memset_s(pstEvtInfo, sizeof(MRS_EVENT_INFO_STRU), 0, sizeof(MRS_EVENT_INFO_STRU));
    ...
    mrsStaEvtPlcSeqAddSelf();
    pstEvtInfo->usSeq = mrsStaEvtGetPlcSeq();
    (hi_void)memcpy_s(pstEvtInfo->ucMeter, HI_METER_ADDR_LEN, pstStaCtx->aucMeter, HI_METER_ADDR_LEN);

    pstPlcFrame->usId = ID_MRS_CMD_EVENT_INF_PLC;
    pstPlcFrame->pucPayload = (HI_PBYTE)pstEvtInfo;
    pstPlcFrame->usPayloadLen = (HI_U16)sizeof(MRS_EVENT_INFO_STRU);

    pstEvtCtx->pstReportData = pstPlcFrame;
...
HI_VOID mrsStaEvtOutProc(HI_VOID)
{
    ...
    else if( (pstCtx->ucStaEnable == HI_FALSE)
    ...
    if(mrsStaEvtGetEvtOut())
    {
        ...
        if (METER_PROTO_698_45 == pstStaCtx->ucProtocol)
        {
            mrsStaEvtReportProc698();
        }
        else
        {
...
#if defined(PRODUCT_CFG_CLTI_SUPPORT_MRS_SWITCH)
    if (mrsIsRealClti() || (STA_TYPE_METER == mrsStaGetType()))
#else
...
// 698.45事件上报处理
HI_U32 mrsStaEvtReportProc698(HI_VOID)
{
    HI_U32 ulRet = HI_ERR_SUCCESS;

    ulRet = mrsStaCreateEvtReportPlc698();
    if (HI_ERR_SUCCESS != ulRet)
    {
        return ulRet;
    }

    return mrsStaSendEvtReportPlc();
}
...
HI_U32 mrsCmdStaEvtInfProc(HI_IN HI_U16 usId, HI_PVOID pReqPacket, HI_U16 usReqPacketSize)
{
    ...
    HI_DIAG_LOG_MSG_E2(MRS_FILE_LOG_FLAG_068, HI_DIAG_MT("[PCO-MR-PW,in handle other pwf]"),pstEvtInf->ulFunCode, powerFailureCtrlInfo->ucWaitAckFlag);

    //停电标志为0时不进行处理
    if ((MRS_EVENT_FUN_REPORT_MODUL == pstEvtInf->ulFunCode || MRS_EVENT_FUN_REPORT_CLT == pstEvtInf->ulFunCode) && 
        MRS_PLC_UP_FLG == pstEvtInf->ulDirBit && MRS_STA_DETECT_POWER_FAILURE_ENABLE == powerFailureNvCtrl->ucPowerFailureReportEnable)
    {
        ...
        if (((pstEvtInf->ulStructLen + pstEvtInf->ulDataLen) != pstReq->usPayloadLen) || 
            ((pstEvtInf->ulDataLen <= DATA_FIX_SIZE) && (MRS_EVENT_FUN_REPORT_MODUL == pstEvtInf->ulFunCode)) || 
            ((pstEvtInf->ulDataLen < DATA_CLT_FIX_SIZE) && (MRS_EVENT_FUN_REPORT_CLT == pstEvtInf->ulFunCode)))
        {
            return HI_ERR_BAD_FRAME;
        }

        mrsGetOthersPowerFailureFrameData(pstEvtInf, HI_NULL, pstReq->is_local_bc_frame);

        if ((HI_FALSE == deviceInfo->device_power_failure) && 
            (HI_FALSE == powerFailureCtrlInfo->isRedayToSendPowerFailure) && 
            (mrsCltPowerOffGetMeterNum() == 0 || (mrsCltPowerOffGetMeterNum() && HI_FALSE == mrsCltPowerGetSendFlag())))
        {
    ...
    if (MRS_EVENT_FUN_ACK == pstEvtInf->ulFunCode && 
        MRS_PLC_DL_FLG == pstEvtInf->ulDirBit && 
        (MRS_STA_DETECT_POWER_FAILURE_ENABLE == powerFailureNvCtrl->ucPowerFailureReportEnable))
    {
        ret = mrsPowerFailureStaRcvAck(pstEvtInf->usSeq);
        if (HI_ERR_SUCCESS == ret)
        {
            return ret;
        }
		...
		//ii采
        ret = mrsCltPowerOnOffRcvAck(pstEvtInf->usSeq);
        if (HI_ERR_SUCCESS == ret)
        {
            return ret;
        }
    ...
    case MRS_EVENT_FUN_CCOBUF_FULL:
        ...
        {
            MRS_StartTimer(MRS_STA_TIMER_EVENT_REPORT,
                pstCtx->ucTimeOutMax * 1000, HI_SYS_TIMER_ONESHOT);
        }
        break;

    case MRS_EVENT_FUN_REPORT_DISABLE:
#if defined(PRODUCT_CFG_CLTI_SUPPORT_MRS_SWITCH)
       if (mrsIsRealClti())
       {
            mrsCltiEvtCtxReset();
            pstCtx->ucCltiEnable = HI_FALSE;
       }
       else if (mrsToolsIsSTA())
#else
	   if (mrsToolsIsSTA())
#endif
       {
            mrsStaEvtCtxReset();
            pstCtx->ucStaEnable = HI_FALSE;
       }
       else
       {
            mrsCltiiEventModuleDeinit();
            pstCtx->ucCltiiEnable = HI_FALSE;
       }
       break;

    case MRS_EVENT_FUN_REPORT_ENABLE:
#if defined(PRODUCT_CFG_CLTI_SUPPORT_MRS_SWITCH)
        if (mrsIsRealClti())
        {
            pstCtx->ucCltiEnable = HI_TRUE;
        }
        else if (mrsToolsIsSTA())
#else
		if (mrsToolsIsSTA())
...
HI_U32 mrsStaEvtMonProc(P_MRS_STA_EVT_CTX pstCtx)
{
    do
    {
        if(STA_TYPE_METER != mrsStaGetType() || mrsStaGetSupportVM() || (METER_PROTO_698_45 == mrsStaGetMeterProtocol()))
        {
            mrsStaEvtClearLastData(&pstCtx->stStaMonInf);
            break;
        }
