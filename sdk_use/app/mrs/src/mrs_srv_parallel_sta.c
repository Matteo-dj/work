...
HI_BOOL mrsParallelBroadCastFiltrate(MRS_STA_SRV_CTX_STRU *pstStaCtx, 
                                            HI_U8 *pData, HI_U16 usLen, HI_U8 ucProtocol);
...
HI_VOID mrsParallelSetStaCtx(MRS_STA_ITEM * queue_item, MRS_PARALLEL_STA_CTX * ctx)
{
    ...
    if (mrsCheck645Proto(queue_item->ucProtocol))
    {
        // 查找到有效帧并依次放入帧上下文中
        while((buf) && (buf_len > 0))
        {
    ...
    {
        ctx->frame_ctx[0].tx_buf = buf;
        ctx->frame_ctx[0].tx_len = buf_len;
        ctx->frame_num = 1;
    }
...
HI_BOOL mrsParallelBroadCastFiltrate(MRS_STA_SRV_CTX_STRU *pstStaCtx, HI_U8 *pData, HI_U16 usLen, HI_U8 ucProtocol)
{
    ...
    (hi_void)memset_s(aucMeter, sizeof(aucMeter), 0, sizeof(aucMeter));
    if (mrsCheck645Proto(ucProtocol))
    {
        ...
        (hi_void)memcpy_s(aucMeter, sizeof(aucMeter), stInf.pucAddr, sizeof(aucMeter));
    }
    else if (METER_PROTO_698_45 == ucProtocol)
    {
        if (HI_ERR_SUCCESS != mrsFind698MeterAddr(pData, usLen, aucMeter))
        {
            return HI_FALSE;
        }
    }

    {
        ...
        result = mrsFindMeterList(&pstStaCtx->stMeterList, aucMeter, &index);
        if(result == HI_FALSE)
        {
            // 不是本站点广播读表命令-过滤
            return HI_TRUE ;
...
/* BEGIN: Modified by liuxipeng/lKF54842,2014/4/26 问题单号:DTS2014042407354 */
    ...
    if(bc_flg)
    {
        ...
        result = mrsParallelBroadCastFiltrate(sta, data, data_len, (HI_U8)t_data->protocol);
        if(result == HI_TRUE)
        {
            return HI_ERR_SUCCESS;
        }
...
/* END  : Modified by liuxipeng/lKF54842,2014/4/26 问题单号:DTS2014042407354 */
    ...
    item->ucProtocol = (HI_U8)t_data->protocol;
...
HI_U32 mrsParallelIrdaPlcProc(HI_U16 usSeq, HI_U8 *pData, HI_U16 usDataLen)
{
    ...
    MRS_PARALLEL_STA_CTX *pstCtx = HI_NULL;
    HI_U8 *pBuf = pData;
    HI_S16 sLen = (HI_S16)usDataLen;
    
    pstCtx = (MRS_PARALLEL_STA_CTX*)mrsToolsMalloc(sizeof(MRS_PARALLEL_STA_CTX));
    if (!pstCtx)
    {
        return HI_ERR_CONTINUE;
    }

    (hi_void)memset_s(pstCtx, sizeof(MRS_PARALLEL_STA_CTX), 0, sizeof(MRS_PARALLEL_STA_CTX));
    ...
    while(pBuf && (sLen > 0))
    {
        ...
        if(HI_ERR_SUCCESS == mrsFind645Frame(pBuf, sLen, &usPos, &usFrameLen))
        {
            if(pstCtx->frame_num >= MRS_PARALLEL_645_FRAME_MAX)
            {
                break;
            }
            
            pstCtx->frame_ctx[pstCtx->frame_num].tx_buf = pBuf + usPos;
            pstCtx->frame_ctx[pstCtx->frame_num].tx_len = (HI_S16)usFrameLen;
            pstCtx->frame_num++;
            
            pBuf += (usPos + usFrameLen);
            sLen -= (HI_S16)(usPos + usFrameLen);            
        }
        else
        {
            break;
...
HI_U32 mrsParallelIrdaPlcResp(MRS_PARALLEL_STA_CTX *pstCtx, HI_U16 usSeq, HI_U8 *pData, HI_U16 usDataLen)
{
    ...
    for (ucIndex = 0; ucIndex < pstCtx->frame_num; ucIndex++)
    {
        ...
        (HI_VOID)mrsIrDA645Dec(pstCtx->frame_ctx[ucIndex].tx_buf, (HI_U16)pstCtx->frame_ctx[ucIndex].tx_len, 
    ...
    pstCtx->protocol = METER_PROTO_645_2007;
