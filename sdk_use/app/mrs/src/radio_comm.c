...
U8 rf_comm_PollCTS(void)
{
    ...
    return 0xFF;
#else
    return radio_comm_GetResponce(0, 0);
#endif
}
 ...
 * @return CTS value
 */
U8 radio_common_SendCmdGetResp(U8 cmdByteCount, U8* pCmdData, U8 respByteCount, U8* pRespData)
{
    radio_common_SendCmd(cmdByteCount, pCmdData);
    return radio_comm_GetResponce(respByteCount, pRespData);
}