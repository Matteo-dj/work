...
HI_PRV HI_BOOL mrsStaAllowSearch(hi_void);
HI_PRV HI_VOID mrsStaStartPeriodSearch(MRS_STA_SEARCH_CTX *pstCtx);
...
{
    if (mrsToolsIsIICollector() && (pstCtx->stSearchCfg.usSearchPeriod > 0))
    {
...
/* BEGIN: PN: DTS2015042907633 MODIFY\ADD\DEL by cuiate/00233580 at 2015/5/2 */
            ...
            if ((METER_PROTO_645_2007 == ucProtocol) || (METER_PROTO_698_45 == ucProtocol))
            {
...
HI_U32 mrsStaSearchAddRmMeter(HI_U8* pucAddr, HI_U8 ucProtocol, HI_U16 usDi97)
{
    ...
    if ((METER_PROTO_645_1997 == ucProtocol) && (0 == usDi97))
    {
        return HI_ERR_FAILURE;
    }
...
HI_BOOL mrsStaIsMeterInSearchResult(MRS_METER_ITEM_S *pstMeterInf, HI_BOOL bReplace)
{
    ...
    for (i = 0; i < pstMeterList->ucMeterNum; i++)
    {
        ...
        {
            if (bReplace || (pstMeterInf->ucProtocol > pstItem->ucProtocol))
            {
                pstItem->ucValidFlag = HI_TRUE;
                pstItem->usOption = pstMeterInf->usOption;
                pstItem->usBaudRate = pstMeterInf->usBaudRate;
                (hi_void)memcpy_s(pstItem, sizeof(MRS_METER_ITEM_S), pstMeterInf, sizeof(MRS_METER_ITEM_S));
                if (pstMeterInf->ucProtocol > pstItem->ucProtocol)
                {
                    pstItem->ucProtocol = pstMeterInf->ucProtocol;
                }
            }
            return HI_TRUE;
        }
        ...
        if (bReplace && (pstItem->ucProtocol == METER_PROTO_645_1997)
...
HI_BOOL mrsStaFindMeterOfSearchResult(HI_U8 *pucMeter, HI_U8 ucProtocol, HI_BOOL bReplace, HI_U16 usDi97)
{
    ...
    for (i = 0; i < pstMeterList->ucMeterNum; i++)
    {
        ...
        // 已存在与当前地址匹配的表，该地址判定为已存在
        if (mrsMeterAddrMatch(pucMeter, pstItem->ucMeterAddr) && (ucProtocol <= pstItem->ucProtocol))
        {
            return HI_TRUE;
        }
...
HI_BOOL mrsStaAllowAddMeter(HI_VOID)
{
    ...
    if(pstSta->plcState == HI_TRUE)
    {
        ...
        if(mrsToolsCheckBCD(ucLocalMac, HI_PLC_MAC_ADDR_LEN))
        {
            return HI_FALSE;
        }
    }
...
{
    MRS_SEARCH_STATUS_NOTIFY_INF *pstNotifyInf = HI_NULL;

    if (ucId >= MRS_STA_NOTIFY_MAX)
	{
		return HI_ERR_FAILURE;
	}

    pstNotifyInf = &(mrsStaSearchCtx()->stStatusNotify[ucId]);
...
HI_U32 mrsStaSearchStartNotify(HI_VOID)
{
	...
	for (ucIndex = 0; ucIndex < MRS_STA_NOTIFY_MAX; ucIndex ++)
	{
	    MRS_SEARCH_STATUS_NOTIFY_INF *pstNotifyInf = &(mrsStaSearchCtx()->stStatusNotify[ucIndex]);
...
HI_U32 mrsAssetNumberAsMeterInsert(HI_VOID)
{
	...
	memset_s(&stNv, sizeof(stNv), 0,sizeof(stNv));
	memset_s(&stMeterInf, sizeof(stMeterInf),0, sizeof(stMeterInf));

	if (HI_ERR_SUCCESS == HI_MDM_NV_Read(HI_NV_FTM_COLLECT_PARAM_FILE, &stNv, sizeof(stNv))
		&&(!mrsToolsCollectorAddr(stNv.CLTAddress)) )
	{
		memcpy_s(stMeterInf.ucMeterAddr, HI_METER_ADDR_LEN, stNv.CLTAddress, HI_METER_ADDR_LEN);
	}
	else
	{
		//HI_MDM_GetLocalMacAddr(stMeterInf.ucMeterAddr);
		return HI_ERR_SUCCESS;
	}
 
    stMeterInf.ucProtocol = METER_PROTO_645_2007;
    stMeterInf.ucValidFlag = HI_TRUE;
    stMeterInf.usOption = 0;
