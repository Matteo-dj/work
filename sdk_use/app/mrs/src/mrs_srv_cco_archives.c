...
{
	MRS_ARCHIVES_NODE_STRU * pstNode=(MRS_ARCHIVES_NODE_STRU *)mrsFixedMemGetItem((MRS_FIXED_MEM_HEAD_PTR)mrsArchivesCtx()->pMeterInf, usIndex);
	if(pstNode && pstNode->ucProtocol>=METER_PROTO_SINGLEWAY_WATERMETER)
	{
		pstNode->ucRead = 1;
	}
	return pstNode;
...
MRS_ARCHIVES_NODE_STRU * mrsSrvArchiversQueryByAddr(HI_PBYTE pMeterAddr)
{
    ...
    mrsMeterAddrPreProc(aucMeterAddr, pMeterAddr, METER_PROTO_645_1997);
...
HI_BOOL mrsSrvArchivesIsExsit(HI_U8 aucMeterAddr[HI_METER_ADDR_LEN])
{
    MRS_METER_ARCHIVES_STRU *pstArchives = mrsArchivesCtx();
    HI_U16 i;
    ...
    for (i = 0; i < pstArchives->usMeterNum; i++)
    {
        ...
        if (mrsToolsMemEq(pstTemp->aucNodeAddr, aucMeterAddr, HI_METER_ADDR_LEN))
        {
            return HI_TRUE;
        }
    }
