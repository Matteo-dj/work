...
//#define LED_UART_RTX            HI_LED_IDX_4   //485数据正在通信
...
#define MRS_COLLECTOR_RUN_LED_MODE_EXTEND       (0)
#define MRS_COLLECTOR_RUN_LED_MODE_STANDARD     (1)

typedef struct
{
    HI_U8 ucRecordTimes;        //记录
    HI_U8 ucReserved[3];           //预留
    HI_U32 ulTotalTimes;        //发生总次数
    MRS_RS485_MULTI_CONNECT_ITEM  stRecord[MRS_RS485_MULTI_CONNECT_MAX_TIMES];        //发生总次数
}MRS_CLTII_R485_MULTI_CONN_STRU;
...
/* END: PN:  MODIFY\ADD\DEL by cuichunyan/00276962 at 2015/10/21 */    
    ...
    HI_U8 ucRunLedMode;     // II采运行灯状态模式: 0表示正常模式(正常状态0.5Hz，搜表中5Hz，没有电表0.25Hz)；1表示国网送检模式(0.5Hz，即运行灯1秒亮1秒灭)
/* BEGIN: Modified by cuichunyan/c00276962 2016/11/10  问题单号:cltii R485 MultiConnect */ 
    HI_U8  ucR485Monitor;        //II采485混接开关    0:关闭 ；1:打开；默认打开
	HI_U8 ucReserved[3];   	 //
	
    MRS_CLTII_R485_MULTI_CONN_STRU stMultiConn;
/* END  : Modified by cuichunyan/c00276962 2016/11/10  问题单号:cltii R485 MultiConnect */
} MRS_COLLECTOR_CTX_STRU;


// The meter information of last broadcast reading. Only for the meter which read failed.
typedef struct
{
    HI_U8 bValid     : 1;   // Valid flag
    HI_U8 ucIndex    : 7;   // Current baudrate index
    HI_U8 ucProtocol : 4;   // Protocol
    HI_U8 ucCount    : 4;   // 
    HI_U8 aucMeter[HI_METER_ADDR_LEN];  // Meter address
...
// 作    者: fengxiaomin/f00209182 [2012-08-28]
//*****************************************************************************
HI_U32 mrsSendData2Meter(HI_U8 *pData, HI_U16 usLen, HI_U8 *ucRetry, HI_U32 over_time, HI_U16 usId);
...
HI_U32 mrsCollectorSendMrDataUart(HI_U8 *pCollectorData, HI_U16 usMrDataLen, HI_U32 over_time,HI_U16 usUart);
...
HI_U32 mrsClrRS485MultiConn(HI_VOID);
/* END  : Modified by cuichunyan/c00276962 2016/11/10  问题单号:cltii R485 MultiConnect */

/* BEGIN: Modified by cuichunyan/c00276962 2016/11/11  问题单号:cltii R485 test by BaudRate */  
