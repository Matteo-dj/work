...
HI_PRV HI_U32 mrsCltiiProtoResp(MRS_COLLECTOR_645_FRAME_STRU *pst645Frame, MRS_IR_645_DI_FUN pIrDIProc,
                                   HI_U16 usId, HI_U16 usSeq);
HI_PRV HI_U32 mrsCltiiCheckCltAddr(HI_U8 *pAddr);
HI_PRV HI_VOID mrsCltiiSetType(HI_VOID);
HI_PRV HI_U16 mrsCltiiGetMeterBaudRate(HI_U8 *pucMeter, HI_U8 ucVer);
...
// 作    者: fengxiaomin/f00209182 [2012-08-28]
//*****************************************************************************
HI_U32 mrsSendData2Meter(HI_U8 *pData, HI_U16 usLen, HI_U8 *pucRetry, HI_U32 over_time)
{
    ...
    do
    {
        ...
        if (HI_ERR_SUCCESS == ret)
        {
            ...
            if (MRS_CLTII_TIMING_645_CTRL == *(pData + usPos + MRS_645_FRAME_CTRL_OFFSET))
            {
                ret = mrsCltiiTimingProc(pData + usPos, pCollectorData, pucRetry, over_time);
                break;
            }
#endif
            if (MRS_COLLECTOR_BOARDCAST_TIMEING_CONTROL == *(pData + usPos + MRS_645_FRAME_CTRL_OFFSET)) // 如果是广播校时，则分别以两种波特率发送数据
            {
        ...
        mrsCollectorSendMrPro(pData, usLen);
...
HI_U32 mrsCltiiTimingProc(HI_U8 *pData, HI_U8 *pFrame, HI_U8 *pucRetry, HI_U32 ulTimeout)
{
    ...
    if (usLen > ucAffixLen)
    {
        (hi_void)memmove_s(pFrame + ucAffixLen, usLen - ucAffixLen, pFrame + ucAffixLen + usPos, usFrameLen);
    }
...
HI_VOID mrsCollectorSendMrPro(HI_U8 *data, HI_U16 usDataLen)
{
    ...
    HI_U8 ucVer = MRS_PROTO645_VER_UNKNOWN;
    ...
    do
    {
        ...
        ucVer = mrsToolsGet645Ver(*(data + pos + MRS_645_FRAME_CTRL_OFFSET));
        usBaudrate = mrsCltiiGetMeterBaudRate(pucMeter, ucVer);
    }while(HI_FALSE);

    MRS_ChangeBaudRate((HI_U32)usBaudrate);
}
...
HI_U32 mrsBoardTimingSendPro(HI_U8 *pTimingData, HI_U16 usDataLen, HI_U8 *pucRetry)
{
    ...
    ret = MRS_ChangeBaudRate(usBaudrate);
    if (ret != HI_ERR_SUCCESS)
    {
        return ret;
    }
...
{
/* BEGIN: Added by liuxipeng/KF54842 2014/01/13 问题单号: DTS2014011303842 */
    HI_MDM_LED_Set485Led(MRS_STA_TIME_UART_LED);
/* END:   Added by liuxipeng/KF54842 2014/01/13 */
    return;
}
...
HI_U32 mrsCltiiTransTimingProc(HI_U8 *pData, HI_U16 usDataLen, HI_U8 *pucRetry, HI_U32 ulTimeOut)
{
    ...
    ulRet = MRS_ChangeBaudRate(g_usTimingBaudRateList[ucIndex]);
...
HI_U32 mrsCreateTimingUpFrame(HI_U8 **ppData, HI_U16 *pusDataLen, MRS_MSG_MR_645_DATA_STRU* pstData)
{
    ...
    *ppData = (HI_U8*)mrsToolsMalloc(MRS_PROTO645_DATAGRAM_LEN_MAX);
    ...
    pstFrame->stCtrl = *(MRS_645_CTRL_STRU*)(&ucCtrl);
...
HI_U32 mrsCltiiProtoProc(HI_U8 *pData, HI_U16 usDataLen, HI_U16 usId, HI_U16 usSeq)
{
    ...
    do
    {
        ...
        if (ulRet == HI_ERR_SUCCESS)
        {
            ...
            if (pIrDIProc)
            {
                mrsCltiiProtoResp(pst645Frame, pIrDIProc, usId, usSeq);
...
// 作    者: fengxiaomin/f00209182 [2015-10-31]
//*****************************************************************************
HI_U32 mrsCltiiProtoResp(MRS_COLLECTOR_645_FRAME_STRU *pst645Frame, MRS_IR_645_DI_FUN pIrDIProc,
                               HI_U16 usId, HI_U16 usSeq)
{
    HI_U32 ulRet = HI_ERR_SUCCESS;
    MRS_STA_ITEM stItem;
    ...
    ulRet = mrsStaTransmit2CCO(pData, usDataLen, MRS_PROTO645_VER_2007, &stItem);
...
HI_U16 mrsCltiiGetMeterBaudRate(HI_U8 *pucMeter, HI_U8 ucVer)
{
    ...
    if (MRS_PROTO645_VER_1997 == ucVer)
    {
