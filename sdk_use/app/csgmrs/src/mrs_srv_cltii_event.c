...
HI_U32 mrsCltiiEventModuleInit(HI_VOID)
{
    ...
    mrsStaSearchRegisterCmdNotify(mrsCltiiSearchStartMsgProc, mrsCltiiSearchFinishMsgProc);
...
HI_U32 mrsCltiiEventMonitorProc(MRS_CLTII_EVENT_CTX_STRU *pEvtCtx)
{
    ...
    if (pMeterInf->ucProtocol == MRS_PROTO645_VER_1997)
    {
    ...
    ulRet = mrsCltiiEventSendFrame(pFrame, usFrameLength);
    mrsToolsFree(pFrame);
    if (HI_ERR_SUCCESS != ulRet)
    {
        mrsCltiiEventMonitorNextItem();
...
HI_U32 mrsCltiiEventSendFrame(HI_U8 *pFrame, HI_U16 usFrameLength)
{
...
HI_U32 mrsCltiiEventFrameRx(HI_U8 *pFrame, HI_U16 usFrameLength)
{
    ...
    do
    {
        ...
        if (MRS_PROTO645_VER_1997 == pstMeterInf->ucProtocol)
        {
...
HI_U32 mrsCltiiEventMonitorItemProc(MRS_CLTII_EVENT_CTX_STRU *pstEvtCtx)
{
    ...
    while (ucMeterIndex < ucMeterTotal)
    {
        ...
        if (MRS_PROTO645_VER_1997 == pMeterInf->ucProtocol)
        {
...
HI_U32 mrsCltiiEventUpdateResult(MRS_CLTII_EVENT_METERLIST *pstMeterList)
{
    ...
    for (i = 0; i < pstMeterList->ucMeterTotal; i++)
    {
        ...
        if (pstNvInf)
        {
            ...
            if (MRS_PROTO645_VER_2007 == pMeterInf->ucProtocol)
            {
...
HI_U32 mrsCltiiEventNvMeterInsert(NV_APP_CLTII_EVENT_ITEM *pstItem)
{
    ...
    if (MRS_PROTO645_VER_2007 == pstMeter->ucProtocol)
    {
        ucWidth = MRS_CLTII_EVENT_TIMES_WIDTH;
        ulInvalid = MRS_CLTII_EVENT_INVALID_NV_TIMES;
    }
    else if (MRS_PROTO645_VER_1997 == pstMeter->ucProtocol)
    {
...
HI_U32 mrsCltiiEventItemRxAckUpdate(NV_APP_CLTII_EVENT_ITEM *pstEvent, HI_U8 ucMeterIndex, HI_U8 ucEventIndex, HI_U8 pucChangeFlag[])
{
    ...
    if (MRS_PROTO645_VER_2007 == pstMeterList->astMeterInf[ucMeterIndex].ucProtocol)
    {
        ulRet = mrsCltiiEvent07ItemUpdate(pstEvent, ucMeterIndex, ucEventIndex, pucChangeFlag);
    }
    else if (MRS_PROTO645_VER_1997 == pstMeterList->astMeterInf[ucMeterIndex].ucProtocol)
    {
...
HI_U32 mrsCltiiEventPlcProc(HI_U8 *pData, HI_U16 usDataLen, HI_U16 usId, HI_U16 usSeq)
{
	...
	ulRet = mrsCltiiEventRespProc(pData + usPos, usFrameLen, usId, usSeq);
...
// 作    者: fengxiaomin/00209182 [2015-12-01]
//*****************************************************************************
HI_U32 mrsCltiiEventRespProc(HI_U8 *pFrame, HI_U16 usFrameLen, HI_U16 usId, HI_U16 usSeq)
{
	HI_U32 ulRet = HI_ERR_SUCCESS;
	HI_U8 ucFlag = 0;
	HI_U8 *pData = HI_NULL;
	...
	do
	{
		...
		if (HI_ERR_SUCCESS == ulRet)
		{
			ulRet = mrsCltiiEventRespCCO(p645Frame, us645Len, usId, usSeq);
		}
	} while (0);
...
HI_U32 mrsCltiiEventQueryProc(HI_U8 *pFrame, HI_PVOID *ppData, HI_U8 *pucDataLen)
{
	...
	for (ucIndex = 0; ucIndex < MRS_MAX_METERNUM; ucIndex++)
	{
		if (pstMeterList->aucEventInf[ucIndex])
		{
			...
			{
                (hi_void)memcpy_s(&pBuf[ucOffset], ucBufLen - ucOffset, pstMeterList->astMeterInf[ucIndex].aucAddr, HI_METER_ADDR_LEN);
				ucOffset += HI_METER_ADDR_LEN;
...
// 作    者: fengxiaomin/00209182 [2015-07-23]
//*****************************************************************************
HI_U32 mrsCltiiEventRespCCO(HI_U8 *pData, HI_U16 usDataLen, HI_U16 usId, HI_U16 usSeq)
{
    HI_U32 ulRet = HI_ERR_SUCCESS;
	...
	if (HI_ERR_SUCCESS == mrsCltiiCheckMeterAddr(pData + MRS_645_FRAME_METERADD_OFFSET))
	{
		...
		if (HI_ERR_SUCCESS != ulRet)
		{
			ucProtocol = MRS_PROTO645_VER_2007;
		}
	}
