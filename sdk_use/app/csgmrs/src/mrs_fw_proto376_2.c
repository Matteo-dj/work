...
//*****************************************************************************
HI_U32 mrs3762ProtoBuffer(MRS_ONE_RAW_FRAME_STRU ** ppstFrame, HI_U16 usFrameType)
{
    ...
    while (us3762InputLen > 0)
    {
        ...
        {//找到帧
            ...
            {
                if (mrsToolsCheck3762Frame(puc3762InputBuffer, usFrameLen, usFrameType) == HI_ERR_SUCCESS) 
                {
                    if (pstTempFrame)
                    {
...
HI_VOID mrs3762ProtoStream2Cache(HI_IN HI_PBYTE pucStream, HI_U16 usStreamLen)
{
    MRS_3762_PROTO_CTX_STRU * pst3762Ctx = mrs3762GetContext();
    ...
    if (usStreamLen <= PRODUCT_CFG_MRS_3762_BUFFER_BYTE_MAX)
    {
        // 将数据放入缓冲区
                       ...
                       HI_OUT MRS_3762_FRAME_STRU** ppstFrame )
{
    ...
    ret = mrsToolsCheck3762Frame(pucDatagram, usDatagramSize, MRS_CCO_FRAME_TYPE_3762);
