...
HI_U32 mrs645ProtoInit(HI_IN HI_OUT MRS_645_PROTO_CTX_STRU* pstCtx)
{
    ...
    pst645Ctx->pucRcvBuffer = mrsToolsMalloc(MRS_PROTO645_DATAGRAM_LEN_MAX*2);
    ...
    (hi_void)memset_s(pst645Ctx->pucRcvBuffer, MRS_PROTO645_DATAGRAM_LEN_MAX*2, 0, MRS_PROTO645_DATAGRAM_LEN_MAX*2);
    ...
    while (usInputLen > 0)
    {
        ...
        {//找到帧
            ...
            {//包不完整
                ret = HI_ERR_CONTINUE;
                int ret1 = 0;
                ret1 = memmove_s(pstCtx->pucRcvBuffer, MRS_PROTO645_DATAGRAM_LEN_MAX*2, pucInputBuffer, usInputLen);
                                        ...
                                        HI_OUT MRS_ONE_RAW_FRAME_STRU* pstFrame, HI_U8 buf_id)
{
    ...
    // 判断缓冲区是否已满
    if ((usStreamLen + pst645Ctx->usRcvBufferSize) > MRS_PROTO645_DATAGRAM_LEN_MAX)
    {
        mrs645ProtoStreamInputCheck(HI_TRUE, buf_id);
        return HI_ERR_BAD_DATA;
    }

    (hi_void)memcpy_s(pst645Ctx->pucRcvBuffer + pst645Ctx->usRcvBufferSize,
        MRS_PROTO645_DATAGRAM_LEN_MAX * 2 - pst645Ctx->usRcvBufferSize, pucStream, usStreamLen);
    pst645Ctx->usRcvBufferSize = (HI_U16)(pst645Ctx->usRcvBufferSize + usStreamLen);
    ret = mrs645ProtoBuffer(pst645Ctx,pstFrame);
...
HI_BOOL mrsMeterAddrMatch(HI_U8 *pSrcAddr, HI_U8 *pDstAddr)
{
    ...
    for (i = HI_METER_ADDR_LEN - 1; i >= 0; i--)
    {
        if (pSrcAddr[i] != MRS_645_METER_ADDR_WILDCARDS)
        {
            break;
        }
    }
...
HI_U32 mrsIsSameAsBufferBbu(HI_U16 usDatagramSize, HI_U8 *pucDatagram, HI_U8 buf_id)
{
    ...
    if ((usDatagramSize > MRS_PROTO645_DATAGRAM_LEN_MAX*2) 
        || !pst645Ctx
    ...
    if (pst645Ctx->usDagramSize != usDatagramSize)
    {
        ret = HI_ERR_FAILURE;
    }
...
HI_U32 mrsAddFrameToBufferBbu(HI_U16 usDatagramSize, HI_U8 *pucDatagram, HI_U8 buf_id)
{
    ...
    (hi_void)memcpy_s(pst645Ctx->pucRcvBuffer, MRS_PROTO645_DATAGRAM_LEN_MAX*2, pucDatagram, usDatagramSize);
    pst645Ctx->usDagramSize = usDatagramSize;
    return HI_ERR_SUCCESS;
}
