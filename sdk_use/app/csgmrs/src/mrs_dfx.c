...
#endif
    DFX_OBJ_DEFAULT(HI_DSID_APP_CSG_PYSICAL_MODE_INF, &dfx_module.app_csg_physicalmode_inf),
...
HI_U32 mrsDfxChlPlcRx(HI_DMS_CHL_RX_S * rx, HI_BOOL msg_proc)
{
    ...
    (hi_void)memcpy_s(plc_inf->src_mac, HI_PLC_MAC_ADDR_LEN, rx->ucSrcMac, HI_PLC_MAC_ADDR_LEN);
    plc_inf->rx_len = rx->usPayloadLen;
    
    (hi_void)memset_s(plc_inf->rx_buf, sizeof(plc_inf->rx_buf), 0, sizeof(plc_inf->rx_buf));
    (hi_void)memcpy_s(plc_inf->rx_buf, MAX_BUF_LEN,
                   rx->pPayload,
...
/* END  : Added by liuxipeng/lkf54842 2014/03/29 问题单号: DTS2014032907816*/
    ...
    (hi_void)memcpy_s(plc_inf->err_src_mac, HI_PLC_MAC_ADDR_LEN, rx->ucSrcMac, HI_PLC_MAC_ADDR_LEN);
    ...
    (hi_void)memcpy_s(plc_inf->rx_err_buf, MAX_BUF_LEN,
                   rx->pPayload,
                            ...
                            HI_DMS_CHL_TX_ADDR_S *addr, HI_BOOL msg_proc)
{
    ...
    (hi_void)memcpy_s(plc_inf->dest_mac, HI_PLC_MAC_ADDR_LEN, addr->ucDestMac, HI_PLC_MAC_ADDR_LEN);

    plc_inf->tx_len = tx->usDataSize;
    (hi_void)memset_s(plc_inf->tx_buf, sizeof(plc_inf->tx_buf), 0, sizeof(plc_inf->tx_buf));
    (hi_void)memcpy_s(plc_inf->tx_buf, MAX_BUF_LEN,
                    tx->ucData, 
                                ...
                                HI_U32 err_cause, HI_BOOL msg_proc)
{
    ...
    (hi_void)memcpy_s(plc_inf->fail_dest_mac, HI_PLC_MAC_ADDR_LEN, addr->ucDestMac, HI_PLC_MAC_ADDR_LEN);
    ...
    (hi_void)memcpy_s(plc_inf->tx_fail_buf, MAX_BUF_LEN,
                    tx->ucData, 
...
/* END  : Added by liuxipeng/lkf54842 2014/03/29 问题单号: DTS2014032907816*/
    ...
    (hi_void)memcpy_s(uart_inf->tx_buf, MAX_BUF_LEN,
                   data,
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprComTx(SDM_MRS_MR_STATS_TYPE_LR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprComRx(SDM_MRS_MR_STATS_TYPE_LR);
#endif
}
#endif
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprPlcTx(SDM_MRS_MR_STATS_TYPE_LR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprPlcRx(SDM_MRS_MR_STATS_TYPE_LR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprComTx(SDM_MRS_MR_STATS_TYPE_PR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprComRx(SDM_MRS_MR_STATS_TYPE_PR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprPlcTx(SDM_MRS_MR_STATS_TYPE_PR);
#endif
}
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO) && defined(PRODUCT_CFG_SUPPORT_SDM_MRS_LOG)
    mrsLogLrprPlcRx(SDM_MRS_MR_STATS_TYPE_PR);
#endif
}
...
HI_BOOL mrsIsTestDI(HI_U8 *pData, HI_U16 usDataLen)
{
    ...
    {
        (hi_void)memcpy_s(&ulDI, sizeof(HI_U32), pData + usPos + MRS_645_FRAME_DATA_OFFSET, sizeof(ulDI));
...
HI_VOID mrsStaDfxRmRxErrData(HI_U16 usId, HI_U8 *pData, HI_U16 usDataLen)
{
    ...
    (hi_void)memcpy_s(&ulDI, sizeof(ulDI), pData, sizeof(ulDI));
    switch (ulDI)
    {
    ...
    case MRS_DAYFREEZE_DATA_DI:
        ...
        if (MRS_DAYFREEZE_TIME_DI == ulDI)
        {
            ...
            (hi_void)memcpy_s(ucTime + 1, sizeof(ucTime) - 1, pData + sizeof(ulDI), sizeof(ucTime) - 1);//日冻结时间内没有秒
...
HI_VOID mrsDfxRmPlcUpAfn(HI_U16 usId, HI_U8 *pData, HI_U32 ulRet)
{
    ...
    if (usId == ID_MRS_CMD_DATA_TRANSMIT_AFN13_PLC)
    {
        ...
        mrsDfxRmPlcTx(pData + sizeof(MRS_DATA_TRANSMIT_AFN13_UP_STRU), pstAfn13->data_len, ulRet);
    }
    else if (usId == ID_MRS_CMD_DATA_TRANSMIT_AFN14_PLC)
    {
        ...
        mrsDfxRmPlcTx(pData + sizeof(MRS_DATA_TRANSMIT_AFN14_UP_STRU), pstAfn14->data_len, ulRet);
    }
    else if (usId == ID_MRS_CMD_PARALLEL_DATA_PLC)
    {
        MRS_DATA_TRANSMIT_AFNF1_UP_STRU *pstAfnF1 = (MRS_DATA_TRANSMIT_AFNF1_UP_STRU*)pData;
        mrsDfxRmPlcTx(pData + sizeof(MRS_DATA_TRANSMIT_AFNF1_UP_STRU), pstAfnF1->data_len, ulRet);
    }
#else
    HI_UNREF_PARAM(usId);
    {
        MRS_CSG_TRANS_XR_UL_STRU * pstTransmit = (MRS_CSG_TRANS_XR_UL_STRU *)pData;
        mrsDfxRmPlcTx(pData +sizeof(MRS_CSG_TRANS_XR_UL_STRU), pstTransmit->usDataLen, ulRet);
    }
#endif
}
#endif
...
HI_VOID mrsDfxRmPlcUpRx(HI_U16 usId, HI_U8* pData, HI_BOOL ucExceedFlag)
{
    ...
    HI_UNREF_PARAM(usId);
    {
        MRS_CSG_TRANS_XR_UL_STRU * pstXrUlFrame = (MRS_CSG_TRANS_XR_UL_STRU *)pData;
        pBuffer=(HI_U8*) pstXrUlFrame+sizeof(MRS_CSG_TRANS_XR_UL_STRU);
        usBufferLen= pstXrUlFrame->usDataLen;
    }
#endif   
...
HI_VOID mrsDfxRmPlcDlRx(HI_U16 usId, HI_U8* pData, HI_BOOL bTimeoutFlag)
{
    ...
    {
        MRS_CSG_TRANS_XR_DL_STRU * pstTransmit = (MRS_CSG_TRANS_XR_DL_STRU *)pData;
        pBuffer = (HI_U8*)pstTransmit +sizeof(MRS_CSG_TRANS_XR_DL_STRU);
        usBufferLen = pstTransmit->usDataLen;
    }
#endif
#if defined(PRODUCT_CFG_PRODUCT_TYPE_STA)
    if (!bTimeoutFlag)
...
HI_VOID mrsDfxPowerOnGetTime(HI_U8 clockBuf[6])
{
    ...
    if (!ucFirst)
    {
        (hi_void)memcpy_s(pstTimeInf->ucFirstClock, sizeof(pstTimeInf->ucFirstClock), clockBuf, sizeof(pstTimeInf->ucFirstClock));
    }
    else
    {
        (hi_void)memcpy_s(pstTimeInf->ucLastClock, sizeof(pstTimeInf->ucLastClock), clockBuf, sizeof(pstTimeInf->ucLastClock));
    }
}
...
HI_VOID mrsDfxCcoEvtBakSave(HI_U16 usBakTime)
{
    ...
    (hi_void)memcpy_s(&dfx_module.app_cco_evt_inf_bak.stDfxCcoEvtInf[1],
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S),
                    &dfx_module.app_cco_evt_inf_bak.stDfxCcoEvtInf[0], 
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S));
    (hi_void)memcpy_s(&dfx_module.app_cco_evt_inf_bak.stDfxCcoEvtInf[0], 
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S),
                    &dfx_module.app_cco_evt_inf.stDfxCcoEvtInf[0], 
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S));
    (hi_void)memcpy_s(&dfx_module.app_cco_evt_inf.stDfxCcoEvtInf[0],
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S),
                    pstEvtInf,
                    sizeof(HI_DFX_APP_CCO_EVT_INF_S));
...
HI_VOID mrsDfxEvtJoinCcoQueueFailCnt(HI_VOID)
{
    ...
    pstEvtInf->ucJoinCcoQueueFailCnt++;
}
...
HI_U32 mrsDfxCcoSaveMACReportChgEntry(HI_MDM_NETWORK_TOPOCHG_ENTRY_S* pstEntry)
{
    ...
    (hi_void)memcpy_s(pstDfxEntry->aucMac, HI_PLC_MAC_ADDR_LEN, pstEntry->aucMac, HI_PLC_MAC_ADDR_LEN);
...
#define mrsDfxGetTestModeInf() (&dfx_module.app_csg_physicalmode_inf)

HI_VOID mrsDfxTestModeCnt(HI_U8 ucMode)
{
    HI_DSID_APP_CSG_PYSICAL_MODE_INF_S_CSG *pstInf = mrsDfxGetTestModeInf();
    pstInf->ulCnt++;
...
HI_VOID mrsDfxTestModeAppBuf(HI_U8 *pAppData, HI_U16 usLen)
{
    ...
    (hi_void)memcpy_s(pstInf->aucAppData, usDataLen, pAppData, usDataLen);
}
...
HI_VOID mrsDfxTestModeSavePayload(HI_U8 *pPayloadData, HI_U16 usLen)
{
    ...
    (hi_void)memcpy_s(pstInf->aucPayLoadData, usDataLen, pPayloadData, usDataLen);
}
#endif
#endif
