...
HI_PRV HI_BOOL mrsIsReadMeterReportItem(HI_VOID *pvItem);
...
HI_BOOL mrsReadMeterStateProcess(HI_INOUT MRS_CCO_SRV_CTX_STRU* pstCcoCtx)
{
    ...
    case EN_MRS_SRV_CCO_READMETER_ASK:               // 询问
        ...
        if (mrsIsCcoUpgStatus() || (!mrsCcoIsTestMode() && !mrsGetCcoPlcChlStatus()))
        {
            // 当前CCO未就绪-不允许询问读表
...
HI_VOID mrsInquireReadMeter(HI_VOID)
{
    ...
    do
    {
        ...
        usLen = 0;
        ret = mrsCreate3762UpFrame(&stAfnData, &pucOutBuf, &usLen, pstCcoCtx->stReadMeterModel.stCurrNode.aucNodeAddr);
        if (ret != HI_ERR_SUCCESS)
        {
            break;
        }
...
HI_VOID mrsAssembleReadMeterData(HI_VOID)
{
    ...
    if(dl_data->ucReadFlg == MRS_ALLOW_READ_CCO) // 可以抄读
    {
        ...
        if(ret == HI_ERR_SUCCESS)
        {
            HI_DIAG_LOG_BUF(MRS_FILE_LOG_BUF_1000, HI_DIAG_MT("MAP-MAC"), mac, 6);
        }
        else
        {
        ...
        transmit_data->protocol      = MRS_STA_PROTOCOL_2007;
        ...
        mrsLogFrameInfStatTx(dl_data->pucData, dl_data->ucDataLen, (HI_U8)(mrsToolsBroadCastMac(mac)));
    #endif
    }
    else    // 抄读失败或抄读成功
    {
...
HI_VOID mrsUpReterMetreData(HI_VOID)
{
    ...
    // 报文长度
    pucBuf[offset++] = (HI_U8)pstUpReadMeter->usDataLen;
    ...
    ret = mrsFind645Frame(pstUpReadMeter->ucData, (HI_S16)pstUpReadMeter->usDataLen, &pos, &frame_len);
    if (HI_ERR_SUCCESS == ret)
    {
        ret = mrsCreate3762UpFrame(&stAfnData, &pucOutBuf, &usBufLen, pstUpReadMeter->ucData + pos + 1);
    }
    else
    {
        ret = mrsCreate3762UpFrame(&stAfnData, &pucOutBuf, &usBufLen, HI_NULL);
    }
...
HI_BOOL mrsIsReadMeterReportItem(HI_VOID *pvItem)
{
    return mrsCcoQueueMatchItem(pvItem, 0x06, 0x02);
}
...
HI_VOID mrsReadMeterStateInit(HI_VOID)
{
    ...
    mrsCcoQueueDeleteItemDefault(mrsIsReadMeterReportItem);
...
/* END:   Modified by fengxiaomin/00209182, 2014/3/27 */
    ...
    if(pstCcoCtx->stReadMeterModel.enCurrState == EN_MRS_SRV_CCO_READMETER_WAIT_PLC_REPLY)
    {
        ...
        if(pstCcoCtx->stReadMeterModel.pstPlcDataDl->pucPayload != HI_NULL)
        {
            ...
            if(pstCcoCtx->stReadMeterModel.pstPlcDataDl->usPayloadLen == sizeof(MRS_DATA_TRANSMIT_AFN14_DL_STRU) + plcDlData->data_len){
...
HI_U32 mrsReadMeterFindValidNode(MRS_CCO_SRV_CTX_STRU * pstCcoCtx)
{
    ...
    {
        mrsForceSwitchPause();
        mrsReportRouterInfs(MRS_CCO_READ_METER_TASK_END); //抄表任务结束
