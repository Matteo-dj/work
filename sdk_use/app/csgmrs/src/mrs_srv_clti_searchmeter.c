...
{    
    ...
    if (MRS_PROTO645_VER_1997 == pstSearchCtx->stSearchCfg.ucNvProtocol)
    {
...
/* END:   PN: DTS2015040900760 MODIFY\ADD\DEL by cuiate/00233580 at 2015/4/7 */
    ...
    if (pstFrame->ucVer == MRS_PROTO645_VER_2007)
    {
    ...
    else if (pstFrame->ucVer == MRS_PROTO645_VER_1997)
    {
...
/* END:   PN: DTS2015040900760 MODIFY\ADD\DEL by cuiate/00233580 at 2015/4/7 */
    {
        ...
        ulRet = mrsCltiSearchMeterInsert(pstSmCtx, pstSmCtx->stSearchCfg.aucNvMeter, pstSmCtx->stSearchCfg.ucNvProtocol);
        if ((HI_ERR_SUCCESS == ulRet) && mrsStaAllowAddMeter())
        {
            ...
            MRS_TOOLS_FE_TO_00(aucMac[MRS_MAC_ADDR_INDEX_5]);
...
/* BEGIN: PN: DTS2015040900760 MODIFY\ADD\DEL by cuiate/00233580 at 2015/4/7 */
    ...
    if (mrsToolsNormalAddr(aucAddr) && 
        ((MRS_PROTO645_VER_1997 == stNv.protocol) || (MRS_PROTO645_VER_2007 == stNv.protocol)))
/* END:   PN: DTS2015040900760 MODIFY\ADD\DEL by cuiate/00233580 at 2015/4/7 */
    {
...
HI_U32 mrsCltiSaveSmResult(HI_U8* pucData)
{
    ...
    HI_U8 ucProtocol = MRS_PROTO645_VER_UNKNOWN;
    HI_U8 ucSaved = 0;
    HI_U8 ucIndex = 0;
    ...
    for (ucIndex = 0; ucIndex < ucCurrent; ++ucIndex)
    {
        ...
        mrsCltiSearchMeterInsert(pstSmCtx, pucMeterAddr, ucProtocol);
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CSG_GD)
    if (bTryFirstFlag && mrsToolsNormalAddr(pstCtx->stSearchCfg.aucNvMeter))
    {
        mrsStaSearchSetFirstFlag(HI_TRUE);
        mrsCsgCltISetStatus(MRS_CSG_CLTI_STATUS_FIRST_07);
        return mrsCsgCltITryFirst();
    }
...
{
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CSG_GD)
    HI_UNREF_PARAM(pstCtx);
    return HI_ERR_SUCCESS;
#else
    return mrsCltiSmStopCmd(pstCtx);
#endif
}
