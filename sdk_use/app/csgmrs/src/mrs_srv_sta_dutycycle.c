...
{
    if (mrsToolsNormalAddr(pstCltParam->meter) && (MRS_PROTO645_VER_2007 == pstCltParam->protocol))
    {
...
HI_U32 mrsStaDutyCycleSendFrame(HI_PBYTE pFrame, HI_U16 usFrameLength)
{
    ...
    pstItem->time_out = MRS_MS_TO_100MS(MRS_RM_METER_OVERTIME_DEFAULT);
...
HI_U32 mrsStaDutyCyclePowerInfFrameRx(HI_U8 *pFrame, HI_U16 usFrameLength)
{
    ...
    (HI_VOID)HI_MDM_MakeCalendarToSecond(&stTime, &pstCtx->ulMeterPowerOnTime);

    if (pstCtx->ulMeterCurTime - pstCtx->ulMeterPowerOnTime > MRS_SECONDS_PER_MINUTE)
    {
