...
// 调用要求: TODO: ...
// 调用举例: TODO: ...
// 作    者: fengxiaomin/00209182 [2017-4-5]
//*****************************************************************************
HI_VOID mrsPhyTestTimeoutProc(HI_VOID)
{
    HI_U8 test_mode = HI_MDM_GetPhysicalTestMode();

    if (test_mode == PHYSICAL_TEST_MODE_LOOPBACK)//物理层回环测试
    {
        HI_MDM_ExitPlcTest();
		mrsSrvCaptureDeinit();
        mrsDfxTestModeExit(APP_CSG_MODE_PHYSICAL_LOOPBACK, test_mode);
    }
    else if (test_mode == PHYSICAL_TEST_MODE_TRANSIMIT)//物理层透传转发
    {
        HI_MDM_ExitPlcTest();
        mrsSrvCaptureDeinit();
        mrsDfxTestModeExit(APP_CSG_MODE_PHYSICAL_TRANSIMIT, test_mode);
    }
}

//*****************************************************************************
// 函数名称: mrsEnterTestModeProc
// 功能描述: 进入测试模式超时处理
//
// 参数说明:
//   无
...
{
    if (!((HI_MDM_GetPhysicalTestMode()) == PHYSICAL_TEST_MODE_TRANSIMIT))
    {
        mrsSrvCaptureDeinit();
    }
}
