...
HI_U32 mrsDut645_DI7EFF1206(HI_VOID *param)
{
    ...
    do
    {
        ...
        if (HI_ERR_SUCCESS != ret)
        {
            stData.RltCode= RET_CODE_ERR_PARSE;
            break;
        }
        ...
        if (HI_ERR_SUCCESS != ret)
        {
            stData.RltCode= RET_CODE_ERR_MAC_ADDRSS;
            break;
        }
        ...
        if (HI_ERR_SUCCESS != ret)
        {
            stData.RltCode= RET_CODE_ERR_USER_VER;
            break;
        }
        (hi_void)memset_s(&stManuInfo, sizeof(HI_NV_FTM_MANUFACOTR_ID_S), 0, sizeof(HI_NV_FTM_MANUFACOTR_ID_S));
        ret = HI_MDM_NV_Read(HI_NV_FTM_MANUFACOTR_ID, (HI_PVOID)&stManuInfo, sizeof(stManuInfo));
        if (HI_ERR_SUCCESS != ret)
        {
            stData.RltCode= RET_CODE_ERR_SZ;
            break;
        }
        ...
        if ((HI_ND_SYS_STATUS_NORMAL == g_st_equip_main_info.ulInitDutMode) && (mrsToolsIsSTA() || mrsToolsIsCLTI()))
        {
...
HI_U32 mrsDut645_DI7EFF1208(HI_VOID *param)
{
    ...
    status = (sta->plcState || (HI_TRUE == HI_MDM_IsStaJoinNet())) ? HI_ERR_SUCCESS : HI_ERR_FAILURE;
...
{
    HI_PBYTE pOutBuff = HI_NULL;
    HI_U16 usOutLen = 0;
    HI_U32 ulRet = HI_ERR_SUCCESS;
    EQUIP_ZERO_CROSS_REQ_ACK_STRU stResult = {0};
    MRS_CSG_IND_FRAME_STRU stIndFrame = {0};

    (hi_void)memset_s(&stResult, sizeof(stResult), 0, sizeof(EQUIP_ZERO_CROSS_REQ_ACK_STRU));
    (hi_void)memset_s(&stIndFrame, sizeof(stIndFrame), 0, sizeof(stIndFrame));

    if (pstResult)
    {
        stIndFrame.pAppData = (HI_U8 *)pstResult;
    }
    else
    {
        stResult.ret = ulResult;
        stIndFrame.pAppData = (HI_U8 *)&stResult;

        if (ulResult == HI_ERR_TIMEOUT) {
            HI_MDM_CollectNtb(HI_FALSE, 0, 0, 0, HI_NULL);
        }
    }

    stIndFrame.ucAfn = MRS_CSG_AFN_DEBUG_F0H;
    stIndFrame.ulDI = MRS_CSG_DI_IND_ZERO_CROSS;
    stIndFrame.ucPrm = MRS_CSG_PLC_CTRL_PRM_SLAVE;
    stIndFrame.ucAddrFlag = PROTOCSG_CTRL_ADD_BIT_N;
    stIndFrame.usAppDataLen = sizeof(EQUIP_ZERO_CROSS_REQ_ACK_STRU);
    ulRet =  mrsCreateCsgFrame(&stIndFrame, &pOutBuff, &usOutLen);
    if ((ulRet == HI_ERR_SUCCESS) && (pOutBuff != HI_NULL))
    {
        MRS_SendMrData(pOutBuff, usOutLen, HI_DMS_CHL_UART_PORT_APP);
    }
...
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO)
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CSG_GD)
    mrsDutCsgReplyZeroCross(result, pstResult);
#else
...
HI_VOID mrsZeroCrossCheckNotify(HI_U32 ulRet, HI_U8 ucLevel, HI_U32 ntb[EQUIP_ZERO_NTB_NUM])
{
    ...
    if (pstResult)
    {
        ...
        (hi_void)memcpy_s(pstResult->ntb, sizeof(pstResult->ntb), ntb, EQUIP_NTB_NUM_DUT * sizeof(HI_U32));
    }

    stMsg.ulMsgId = EN_MRS_FW_MSG_ZERO_CROSS_NOTIFY;
    stMsg.ulParam[0] = (HI_U32) pstResult;
    if (HI_ERR_SUCCESS != mrsSendMessage2Queue(&stMsg))
    {
        mrsToolsFree(pstResult);
    }
}
